...
5. **Every program gets ONE brief, capped at 400 lines** (`scripts/codex-plan` enforces the cap and
   ...
   git keeps them.
6. **Cross-checking earns its cost** on ideas, briefs, and plans — a plan-mode plan goes through
   `scripts/review --author claude` before leaving plan mode; deterministic checks and tests
   outrank model agreement everywhere else — agreement is not evidence.
