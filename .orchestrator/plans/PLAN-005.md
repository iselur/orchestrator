...
ledger_ref: R16
lane: high-assurance
supersedes: PLAN-005 revision 3
---
...
4. **Root-backed per-use authorization.** A break-glass token shall be derived from a root-held secret and obtainable only through an explicit interactive `sudo` invocation of a root-owned issuance helper. Issuance shall require fresh interactive authorization and shall not be available through the dispatch process or the scoped noninteractive redemption permission. The authenticated token fields shall cover, at minimum, the exact spec digest, attempt ID, base SHA, and expiry. Any additional token fields, including version, token ID, repository identity, issuer identity, issue time, reason, and exposure acknowledgement, shall also be authenticated. The root-owned issuance helper shall derive issue time exclusively from the trusted host clock and shall enforce expiry no more than 15 minutes after that trusted-clock issue time; caller-supplied current or issue time shall not control or relax this validation. Malformed, expired, mismatched, unauthenticated, or otherwise invalid tokens shall fail closed.

5. **Atomic root-owned replay prevention.** Immediately after successful token validation and before `claim_slot`, dispatch shall invoke only the narrowly scoped sudoers redemption helper. The root-owned helper shall atomically validate the token against the root-held secret, compare its authenticated spec digest, attempt ID, and base SHA with the current invocation, derive current time exclusively from the trusted host clock inside the helper at redemption time, enforce issue time, expiry, and the maximum validity window using only that trusted clock read, refuse a previously redeemed token, and durably append the redemption to a root-owned append-only ledger before reporting success. Caller-supplied time shall not be an input to any issue-time, expiry, or validity-window decision: the redemption interface shall reject any caller-supplied current-time or `now` parameter. The operator UID shall be unable to modify, truncate, replace, unlink, or delete the ledger or its parent directory. A crash after durable redemption shall spend the token rather than make it reusable. Dispatch shall never validate or redeem a token solely in operator-owned code or storage.
...
18. **Trusted-clock expiry cannot be bypassed.** A regression shall attempt to redeem an expired, unused token while the caller supplies a timestamp that falls inside the token’s authenticated issue/expiry window. Redemption shall refuse because the helper’s own trusted host-clock read at redemption time establishes that the token is expired, and the supplied timestamp shall not enter the decision. Dispatch shall create no durable attempt state or worker-controlled execution and shall remain refused until a fresh token is obtained through a new explicit interactive `sudo` issuance.
...
- The issuance helper reads the trusted host clock inside the root-owned helper, uses that read as the authoritative issue time, and rejects any requested expiry more than 15 minutes after that issue time. Caller-supplied current time or issue time is not accepted as authoritative and cannot control or relax issue/expiry validation.
- The root-held secret and raw keyed authenticator are never written to repository evidence.

Dispatch treats the token as an opaque bearer value. It does not possess the root secret and cannot mint or independently bless tokens. It passes the opaque token plus the current canonical spec digest, attempt ID, and base SHA to the exact redemption command allowed by the scoped-sudoers wrapper. Dispatch does not pass a current-time value for validation, and the redemption command rejects any caller-supplied current-time or `now` parameter.
...
4. Reads current time exclusively from the trusted host clock inside the root helper at redemption time and uses that read as the sole authoritative `now` for issue-time, expiry, and maximum-validity-window enforcement.
...
Caller-supplied time is not an input to the redemption decision and cannot control or relax issue-time, expiry, or maximum-validity-window enforcement. The helper interface rejects a caller-supplied current-time or `now` parameter rather than treating it as authoritative.
...
The security design assumes that the host clock read by the root-owned issuance and redemption helpers is trusted and adequately maintained. Compromise or gross misconfiguration of that clock is outside the authorization mechanism’s protection. In particular, a large backwards host-clock jump can disrupt issue/expiry interpretation, delay effective expiry, or cause fail-closed refusals depending on the resulting relation to authenticated issue time. This is an explicit recorded residual risk, not an unmentioned alternate authority: caller-supplied time remains non-authoritative in all cases.
...
3. Define the canonical break-glass request tuple and invocation attempt-ID generation/transfer. Implement only the unprivileged dispatch side: strict opaque-token input handling, exact invocation-binding-field construction, invocation of the single allowed redemption command without any caller-supplied current-time or `now` parameter, strict receipt parsing, secret redaction, and fail-closed handling. Dispatch shall contain no root secret and no operator-owned fallback validator or ledger.

4. As part of the scoped-sudoers wrapper work queued as baton action 5, install the root-owned interactive issuance helper, root-held secret, root-owned atomic redemption helper, narrowly scoped sudoers rules, and root-owned append-only redemption ledger. Separate interactive issuance authority from noninteractive redemption authority. Require both helpers to derive authoritative current time exclusively from the trusted host clock inside the root-owned helper, reject caller-supplied current-time or `now` parameters, and bind redemption expiry checking to the redemption helper’s own clock read at redemption time. Until this step lands successfully, leave break-glass unavailable and preserve refusal-only behavior.
...
8. Add refusal, token-validation, expiry, caller-supplied-time bypass, mismatch, concurrent redemption, replay, helper-unavailable, single-probe, no-downgrade, and consumer-routing regressions. The caller-supplied-time regression shall present an expired, unused token together with an in-window timestamp, require refusal based on the root helper’s own redemption-time clock read, and prove that a fresh human token is still required afterwards. Add the authorized exposure payload that attempts to forge a token, erase or replace the root ledger, and replay its authorization. Every negative case must include durable-state snapshots, payload markers, exact reason assertions, and a positive control.
...
| Caller supplies an earlier in-window timestamp for an expired, unused token | Attempted expiry bypass | Reject the caller-time parameter; validate expiry only against the root helper’s trusted host-clock read at redemption time; refuse until a fresh token is issued. |
...
| Trusted host clock makes a large backwards jump | Token validity interpretation may be extended or fail closed unexpectedly | Record the clock event as a security-relevant residual, restore trustworthy time, disable break-glass while clock integrity is uncertain, and require fresh issuance after recovery. Never substitute caller time. |
...
A clock-integrity residual also remains: the root helpers depend on the host clock as their trusted time source. A large backwards clock jump can disrupt issue/expiry interpretation, including delaying effective expiry or causing fail-closed refusal depending on the resulting authenticated times. Operators must treat a material backwards jump as a security-relevant host event, suspend break-glass while clock integrity is uncertain, restore trustworthy time, and require fresh interactive issuance afterwards. Caller-supplied time is never a fallback authority and cannot be used to repair, override, or relax the helper’s decision.
...
- The issuance helper derives issue time from its own trusted host-clock read and rejects caller-supplied current or issue time as an authority for the maximum validity-window decision.
- An expired, unused token refuses when the caller supplies a timestamp inside the token’s authenticated issue/expiry window. The redemption CLI shall reject the caller-supplied current-time or `now` parameter; a subsequent normal redemption attempt without that parameter shall still refuse the token as expired based on the root helper’s own clock read at redemption time.
- After that expired-token attempt, dispatch remains refused without durable attempt state or worker-controlled execution. Only a new token obtained through a fresh explicit interactive `sudo` issuance may authorize the next harmless exposure launch.
...
- The scoped redemption permission cannot run the issuance action, read the root secret, select an arbitrary ledger path, supply an authoritative current-time or `now` value, execute a shell, or invoke an arbitrary root command.
...
1. Record OS, kernel, AppArmor userns setting, repository commit, spec digest, D5 probe result, and the host clock source/state used by the root helpers.
...
6. Issue a disposable token, leave it unused until it expires according to the root helper’s trusted host clock, then attempt redemption while supplying a timestamp inside its authenticated validity window. Require refusal, no redemption record, no durable attempt state, and no worker-controlled execution. Retry without the supplied-time parameter and require expiry refusal from the helper’s own redemption-time clock read. Prove dispatch remains refused until a fresh token is obtained through a new explicit interactive `sudo` issuance.
7. Run the forge/erase/replay hostile-exposure regression and prove that every later dispatch refuses until a fresh human token is issued.
8. Prove mapped `unshare` denial only as host background evidence; dispatch must not call `unshare`.
...
Produce one machine-readable report from each environment, bound to the same implementation commit and test digest. The high-assurance validation is complete only when both reports pass, the real privileged break-glass infrastructure has passed the authorization, trusted-clock expired-token bypass, and hostile-exposure cases, and the live canary’s three-marker positive control and sabotage cases pass. A single refusal report, a simulated root helper, caller-controlled time validation, a simulated isolated result, an early success, a skipped worker control read, or a skipped live canary is explicitly `INCOMPLETE`, never `PASS`.
...
- **Dual-validation (high-assurance/control-plane):** Revision 3 stop-rule review returned `BLOCK` with exactly one security-material finding: the root redemption helper accepted caller-supplied current time, permitting an expired unused token to be redeemed with an earlier in-window timestamp. Claude sustained the finding without scaling and declared every other reviewed property internally coherent. Revision 4 review is pending.
...
1. **Finding 1 — sustained-applied.** The frontmatter and sections 3.4–3.5, 3.18, 4.2, 6.3–6.4, 6.8, 7, 8.2, 8.4, 8.7, and 11 were changed. The root-owned issuance and redemption helpers now derive authoritative current time exclusively from the trusted host clock inside the helper. The redemption helper binds issue-time, expiry, and maximum-validity-window checking to its own clock read at redemption time and rejects caller-supplied current-time or `now` parameters, so caller time cannot control or relax the decision. Requirements and validation now include an expired, unused token presented with an in-window caller timestamp; redemption and dispatch must refuse without durable state or worker-controlled execution, and only a fresh token obtained through a new explicit interactive `sudo` issuance may authorize a later exposure dispatch. Section 7 records the trusted-host-clock assumption and large backwards clock jumps as an explicit residual risk. All other revision 3 text and properties remain frozen.

## 12. Authorization (Claude, 2026-07-14) — under the R24 stop-rule

**Adversarial review (fresh-context, stop-rule applied): VERDICT: PASS — no stop-rule finding.**
All five checked properties coherent (frozen decision before durable state and threaded through every
execution consumer with no downgrade path; break-glass crosses a root-owned boundary and resists
minting/alteration/replay by operator-UID payloads; validation is non-vacuous; both environments
executable; baton action 5 is an explicit prerequisite, not an unresolved dependency).

**AUTHORIZED FOR IMPLEMENTATION**, with three advisory amendments dispositioned here (advisories do
not block under R24; these are folded into the spec as binding requirements):

- **A1 (frontmatter duplication):** MOOT — verified: the installed artifact carries exactly one
  frontmatter block and one `revision:` line.
- **A2 (canary control markers) — ACCEPTED, BINDING.** `CONTROL_READ_SUCCEEDED` must be verifiably
  dependent on the RANDOM CONTENTS of the control token (the probe must echo/hash the token's random
  value, compared by the controller), never on marker text alone. Marker text alone would defeat this
  plan's own skip-read sabotage requirement — i.e. it would reintroduce a vacuity hole. The spec MUST
  encode this.
- **A3 (break-glass env hygiene) — ACCEPTED, BINDING.** Break-glass variables/tokens must be removed
  from EVERY child-process environment after mode selection, including the healthy-D5 case where a
  supplied token goes unredeemed (an unredeemed token must never leak into a worker/test environment).

**Authorized revision:** 4 (+ amendments A2/A3, which the implementation spec must carry verbatim).
Silent post-authorization edits void this authorization.
