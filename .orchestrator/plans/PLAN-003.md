...
1. An operator-installed, root-owned authoritative-evidence choke point, installation record, trusted parent snapshot, minimally provisioned gate environment, and a conditional PR/push broker boundary whose authority is enabled only after the operator credential-segregation and real-remote enforcement prerequisite is verified.
...
The root-owned launcher/service is the only path to authoritative evidence roots. It is the only path to authoritative PR/push operations only after the operator prerequisite in R1 has removed or revoked every parallel non-broker write/PR credential available to the operator, the hosting platform has restricted service-controlled refs and authoritative PRs to the broker/machine identity, and the real remote has rejected authenticated non-broker push and PR attempts. Before that prerequisite is verified, PLAN-003 makes no authoritative PR/push choke-point claim. Its guarantee is narrowed to authoritative evidence authority: evidence roots, attestation, review artifacts, and installation records are choke-pointed, while PR/push authority is not yet choke-pointed. That limitation is owned by `machine-account-authority-split`.

In the narrowed state, a direct repository dispatcher run may perform local, non-authoritative work and may still be able to use parallel operator credentials for external PR/push effects. None of its evidence, reviews, attestations, test results, attempt identity, push results, PR results, pathnames, or digests can be adopted or represented as authoritative evidence of gate passage. A commit produced by such a run may be submitted only as untrusted input to a fresh service-controlled attempt. The kernel fails closed if asked to attest, import, label, or otherwise represent a PR produced outside the broker path as authoritative evidence, regardless of whether the hosting platform reports that PR as open, mergeable, CI-passing, or merged.

The operator installs an exact parent and records its commit, tree, trusted-component digests, required-test inventory digest, verdict-schema digest, and gate-environment identity. Each authoritative attempt records that installation-record digest. Before candidate launch, the root-owned launcher measures the installed parent tree and trusted inputs, compares them against the installation record’s recorded commit and digests, and refuses to launch dispatch when they disagree. The launcher is root-owned and outside candidate/operator write authority, so a mismatched or substituted parent cannot be launched through the authoritative path. The dispatcher does not verify its own installation record. A direct non-launcher dispatcher invocation remains possible, but its artifacts remain non-authoritative under the evidence choke point. This is deliberately a simple installed-parent check, not an activation ledger or compare-and-swap state machine.
...
### R1. Authoritative launcher, installed-parent record, evidence choke point, and conditional PR/push authority

The root-owned launcher/service must be the exclusive path to authoritative evidence. It may be represented as the exclusive path to PR/push authority only after an explicit operator prerequisite has removed every parallel non-broker write/PR credential, installed remote-side broker-identity enforcement, and passed a negative authorization test against the real remote. Without that verified prerequisite, the kernel operates in evidence-only authority mode and records that PR/push authority is not choke-pointed.
...
3. **R1.3:** Before attempt creation or candidate launch, the root-owned launcher measures the installed parent tree and trusted inputs selected for launch and rejects unless their commit, tree, trusted-component digests, inventory digest, schema digest, and environment identity match the installation record. Any disagreement causes the launcher to refuse to launch the dispatcher.

4. **R1.4:** Every authoritative attempt manifest records the exact installation-record digest and launcher-measured parent commit/tree. A mismatch produces launcher refusal before dispatcher or candidate work.

5. **R1.5:** Authoritative evidence roots are reachable only through the root-owned launcher/service. Repository dispatchers, candidate processes, `codex-worker`, and ordinary direct shell invocations have no independent authority to allocate, write, import, or adopt authoritative evidence.

6. **R1.6:** Before PR/push authority is described or enabled as choke-pointed, the operator completes and records a credential inventory for every non-broker path capable of repository write, ref creation/update, PR creation, PR mutation, merge, ruleset bypass, or administration. The inventory checks at least:

   - system, global, local, worktree, and URL-scoped Git credential-helper configuration and every referenced helper;
   - Git credential-cache daemons and sockets;
   - plaintext credential files, including Git credential stores and `.netrc`-style files;
   - OS keyrings, secret stores, desktop credential managers, and cached HTTPS credentials;
   - `gh auth` hosts, stored `gh` tokens, and all `GH_*`, `GITHUB_*`, askpass, and equivalent environment-provided credentials;
   - shell startup files, environment managers, terminal/tmux sessions, user services, scheduled jobs, and other operator processes that may retain tokens;
   - PATs, OAuth tokens, GitHub App or equivalent installation/user tokens, deploy tokens, and repository-scoped machine tokens accessible to the operator UID;
   - SSH agents and all loaded identities;
   - default and named SSH private keys, certificates, hardware-backed identities, `IdentityFile`/`IdentityAgent` settings, `Include`/`Match` rules, and host aliases capable of reaching the repository;
   - repository deploy keys and hosting-platform account identities associated with those keys; and
   - remote roles, admin/bypass grants, and cached credentials that could alter the enforcement policy.

   Every discovered non-broker credential with write, push, PR, merge, bypass, or administrative authority for this repository is revoked remotely and removed from the operator environment. Merely unsetting it in the current shell or intercepting `git`/`gh` locally is insufficient.

7. **R1.7:** The hosting platform enforces the boundary independently of the operator UID. Branch protection/rulesets, ref-creation and push restrictions, repository permissions, PR policy, and bypass lists must ensure that:

   - only the broker/machine identity may create or update service-controlled refs;
   - only a broker/machine-identity operation can create or mark a PR as authoritative for the service-controlled path;
   - a non-broker PR targeting the authoritative integration path is rejected by remote policy or is mechanically non-authoritative and unmergeable;
   - `ci` success alone cannot convert a non-broker ref or PR into an authoritative or mergeable service result;
   - the operator’s ordinary identities possess no write, PR-authority, merge, administrative, or ruleset-bypass route around these controls; and
   - remote policy changes require an identity/control not available to the ordinary operator UID or candidate environment.

8. **R1.8:** A negative authorization test executes against the real hosting remote with an authenticated non-broker test identity whose identity is verified before the test. It attempts both:

   - creation or update of a sacrificial service-controlled ref; and
   - creation of an authoritative PR, or conversion of a non-broker PR into the authoritative mergeable path.

   Both operations must be rejected by the remote authorization policy, not by spies, PATH interception, missing credentials, DNS failure, an invalid token, or local wrapper logic. The test verifies the remote created no service-controlled ref, no authoritative or mergeable PR, and no integration-tip change. If the platform cannot enforce or prove either rejection, PR/push authority remains not choke-pointed.

9. **R1.9:** The operator prerequisite produces a root-owned verification artifact binding the repository identity, remote URL/host, broker account or application identity, non-broker test identity, credential-inventory result, revoked credential identifiers or fingerprints where safely recordable, remote ruleset/protection identifiers and digests, bypass-list state, real-remote request/result evidence, test time, and verifier identity. The launcher records this artifact’s digest and authority mode. Before each brokered authoritative external operation, the broker verifies that the selected artifact applies to the current repository and that the relevant remote policy still matches; missing, stale, unreadable, or mismatched state disables authoritative PR/push operations.

10. **R1.10:** Until R1.6–R1.9 are verified, every attempt records `evidence-only` authority mode. In that mode, authoritative evidence authority is choke-pointed, PR/push authority is explicitly not choke-pointed, and the limitation is owned by `machine-account-authority-split`.

11. **R1.11:** In either mode, the kernel refuses to represent a PR produced outside the broker path as authoritative evidence of gate passage. External existence, CI success, mergeability, or merge status cannot substitute for a broker-bound attempt identity, installation-record digest, attestation, review, and authority-mode record.

12. **R1.12:** Direct invocation of repository `scripts/dispatch` leaves authoritative evidence untouched. Push/PR spies must remain at zero in local integration tests, but those spies are supplementary checks only and are never accepted as proof of credential segregation or remote authorization.

13. **R1.13:** No evidence, review output, attestation, test result, attempt identity, push result, PR result, pathname, or digest from a direct run can be copied, linked, imported, or adopted by an authoritative attempt.

14. **R1.14:** A direct-run commit may be readmitted only as untrusted candidate input to a fresh launcher-controlled attempt with a fresh identity, fresh authoritative evidence, fresh trusted tests, fresh review, and, when full PR/push authority mode is enabled, broker-controlled push/ref and PR operations.

15. **R1.15:** The worker and candidate cannot write the installation record, installed parent content, required-test inventory, authoritative evidence directories, credential store, or operator-prerequisite verification artifact.

16. **R1.16:** A candidate change to a required trust test or its trusted helper/fixture requires explicit human authorization naming the exact old and new Git blob IDs before the new blob can appear in a later installed-parent inventory.

17. **R1.17:** The new blob remains supplemental during the candidate attempt that introduces it. It becomes authoritative only after merge, independent human review, explicit old/new-blob authorization, operator installation, and launcher-side installation-record verification.
...
7. **R8.7:** Phase 2 remains blocked until all three PLAN-003 dispatches are merged where applicable, independently reviewed, operator-installed, launcher-side installation-record verified, and validated through the required on-box checks. Completion establishes the authoritative-evidence guarantee. PLAN-003 may additionally record authoritative PR/push authority only when R1.6–R1.9 are currently verified; otherwise Phase 2 inherits the explicit `machine-account-authority-split` limitation and must not treat any external PR or push as authoritative evidence of gate passage.
...
|---:|---|---|---|
| 0 | Root-owned launcher, installation record, authoritative evidence boundary, conditional PR/push broker boundary, parent snapshot, and minimum gate environment | operator-installed trust foundation | Establish the authoritative-evidence choke point; install and record the exact current parent; provision dependencies needed to execute required tests; install and verify the operator credential-segregation and real-remote prerequisite before claiming PR/push authority. |
...
“Completed” means the repository delivery was reviewed and merged where applicable, the exact merged parent was installed by the root-owned launcher, the installation record was generated and verified by the launcher, the on-box positive/negative controls passed, and authoritative evidence operations did not bypass the launcher. PR/push authority is a separate conditional completion property: it is recorded only after credential inventory and revocation, remote enforcement, and the real-remote negative authorization test have passed and produced the verification artifact required by R1.9. Otherwise completion is explicitly `evidence-only`.
...
- the root-owned authoritative attempt-evidence root;
- the root-only PR/push credential store and broker; and
- the root-owned operator-prerequisite verification-artifact directory.
...
- tests dedicated to launcher authority, installed-parent verification, minimum dependency execution, authority-mode handling, and supplementary local broker-call checks.

Credential revocation, hosting-platform permission changes, branch protection/ruleset changes, push restrictions, PR-authority restrictions, bypass-list changes, and the real-remote negative authorization test are operator-controlled external prerequisite actions, not candidate repository path allowances.
...
### 4.3 Root-owned evidence choke point and conditional PR/push authority

The launcher/service always owns:

- authoritative attempt identity allocation;
- authoritative evidence-directory allocation;
- installed-parent selection;
- installation-record generation and verification; and
- trusted controller execution.

The launcher/service conditionally owns:

- PR/push credentials;
- service-controlled refs; and
- brokered push and PR operations.

The conditional ownership is authoritative only after the operator prerequisite is verified. Root ownership of a new broker credential does not revoke or neutralize pre-existing operator Git, hosting-platform, SSH, PAT, OAuth, application, deploy-key, helper, cache, or environment credentials.

The operator prerequisite therefore has both local and remote halves:

1. Inventory, revoke remotely, and remove from the operator environment every non-broker credential or account path with repository write, PR-authority, merge, bypass, or administrative capability.
2. Configure the hosting platform so its branch protection/rulesets, push restrictions, PR policy, repository permissions, and bypass lists restrict service-controlled refs and authoritative PR operations to the broker/machine identity.
3. Execute authenticated non-broker push and PR attempts against the real remote and require remote authorization rejection.
4. Persist the verification artifact and bind its digest and authority mode to authoritative attempts.

The repository dispatcher receives an attempt-bound launch context containing the authoritative attempt identity, exact installation-record digest, and authority mode. It never receives the underlying broker credential.

In `evidence-only` mode, a direct repository invocation lacks a valid authoritative launch context and cannot allocate or write authoritative evidence or request brokered external effects. It may still create local files, processes, worktrees, reviews, commits, pushes, or PRs through credentials outside the broker boundary. Those external effects demonstrate why PR/push authority is not claimed in this mode. They cannot be imported, adopted, labeled, or represented as authoritative gate evidence.

In fully verified mode, the direct invocation still lacks broker context, while the real remote independently rejects non-broker service-ref and authoritative-PR operations. Local push/PR spies provide additional regression coverage for dispatcher behavior but do not prove this authorization property.

Readmission of a direct-run commit creates a new attempt. The launcher ignores all direct-run artifact paths and digests, assigns a fresh identity, reruns required tests, and obtains a fresh review. In fully verified mode it also creates a new service-controlled ref through the broker. Poisoned local artifacts are used in the regression to make accidental adoption detectable.

A PR can be represented as authoritative evidence only when it is bound to the fresh broker-controlled attempt, installation record, attestation, review, service-controlled ref, current authority-mode verification artifact, and broker identity. Any PR outside that chain is external state only, even if it exists, passes `ci`, is mergeable, or has been merged.
...
Before attempt creation or candidate work, the root-owned launcher measures the installed parent tree and its trusted inputs and compares them with the selected record. Missing, malformed, duplicate-keyed, wrong-owner, worker-writable, or mismatched state causes the launcher to refuse to launch the dispatcher.

This verification is performed by the root-owned launcher, which is outside candidate/operator write authority. It prevents a mismatched or substituted parent from being launched through the authoritative-evidence path. It does not make the dispatcher self-verifying: the dispatcher does not verify its own installation record. A direct non-launcher dispatcher invocation remains possible, but it cannot produce adoptable authoritative evidence. Whether it can independently push or create a PR depends on the separately verified credential-segregation and remote-enforcement prerequisite; until that prerequisite passes, PLAN-003 expressly makes no PR/push choke-point claim.
...
8. **Retain the full revision-6 bootstrap verifier and activation protocol.** Deferred by R24 in favor of the authoritative-evidence choke point, simple operator installation, and independent human review. PR/push authority remains conditional on the separate operator credential-segregation and remote-enforcement prerequisite.

9. **Treat root ownership of a new broker credential or zero-call push/PR spies as proof of exclusive PR/push authority.** Rejected because parallel operator credentials can bypass both while the spies remain green.

10. **Rely on local credential removal without remote enforcement.** Rejected because a cached, restored, alternate-host, SSH, or remotely privileged identity can recreate the bypass.

11. **Rely on remote policy without an authenticated real-remote negative test.** Rejected because policy configuration can be incomplete, target the wrong repository/ref, or retain an operator bypass.

12. **Inject a sanitized log tail.** Deferred. The machine attestation summary is the primary reviewer evidence.

13. **Unblock Phase 2 after metrics.** Rejected because corrected metrics do not close the false-pass path.

## 5. Affected boundaries & consumers

| Boundary or consumer | Effect |
|---|---|
| Root-owned launcher/service | Becomes the exclusive authority for attempt identity, authoritative evidence allocation, installed-parent selection and verification, and trusted controller execution. It becomes exclusive PR/push authority only after the operator prerequisite is verified. |
...
| Authority-mode record | States `evidence-only` or fully verified PR/push authority and binds the operator-prerequisite verification-artifact digest. |
| Operator credential environment | Must be inventoried for Git helpers, caches, keyrings, `gh` auth, environment tokens, PAT/OAuth/app tokens, SSH agents and keys, deploy credentials, and remote bypass/admin identities; every non-broker write/PR path must be revoked and removed before full PR/push authority is claimed. |
| Hosting-platform remote policy | Must restrict service-controlled refs and authoritative PR operations to the broker/machine identity where the ordinary operator UID cannot override the boundary. |
| Real-remote negative authorization test | Becomes the primary proof that authenticated non-broker push and authoritative-PR attempts are rejected by the remote. |
| Direct repository dispatcher | May produce only local or external non-authoritative effects without a launcher-issued context; it does not verify its own installation record. Before the operator prerequisite passes, it may still possess parallel PR/push capability, which is a stated limitation rather than a denied fact. |
| Direct-run commit | May be submitted only as untrusted input to a fresh authoritative attempt. |
| Direct-run PR or push | Can never be represented as authoritative evidence of gate passage. In fully verified mode the real remote must reject the service-controlled operation; in evidence-only mode its possible existence is the recorded limitation. |
| Authoritative evidence root | Becomes non-worker-writable and accessible only through the launcher/service. |
| PR/push credential | Remains behind the service and is never passed to repository or candidate processes; its root ownership is necessary but not sufficient without removal of parallel credentials and remote enforcement. |
...
| Attempt manifest | Gains the persisted criteria vector, vector digest, installed-parent bindings, attestation summary, authority mode, and operator-prerequisite artifact digest when present. |
...
| CI | Remains portable validation and cannot substitute for the authoritative on-box launcher boundary or the real-remote authorization test. |
| Phase 2 | Remains blocked until the entire three-dispatch kernel is installed and validated. If the machine-account prerequisite is not verified, Phase 2 inherits evidence-only authority and cannot treat PR/push results as authoritative evidence. |
...
| `machine-account-authority-split` | Owns completion or restoration of the PR/push authority boundary whenever PLAN-003 remains in evidence-only mode. |
...
2. Independently review the root-owned launcher, installation-record format, authoritative evidence permissions, credential broker, authority-mode representation, and operator-prerequisite verification-artifact format.
...
5. Generate the installation record, configure the root-owned launcher to measure and compare the installed parent before launching dispatch, and verify worker write denial across the installed parent, record, environment, authoritative evidence root, credential store, and operator-prerequisite verification-artifact location.

6. Set the initial recorded authority mode to `evidence-only`. Prove the launcher and dispatcher refuse to represent any outside-broker PR or push as authoritative evidence in that mode.

7. Inventory every operator-accessible Git/hosting credential and authorization path listed by R1.6, including helpers, caches, stores, keyrings, `gh` auth, environment tokens, PAT/OAuth/app tokens, SSH agents and keys, deploy credentials, account roles, admin grants, and ruleset bypasses.

8. Revoke remotely and remove locally every non-broker credential or identity path with repository write, PR-authority, merge, administrative, or bypass capability. Re-run the inventory from clean login, service, agent, and shell contexts and require no such path to remain.

9. Configure remote branch protection/rulesets, service-ref creation/update restrictions, repository permissions, PR-authority policy, and bypass lists so only the broker/machine identity can perform service-controlled push and authoritative-PR operations and `ci` alone cannot make a non-broker path authoritative or mergeable.

10. Using an authenticated non-broker test identity, execute sacrificial push and authoritative-PR attempts against the real remote. Require remote authorization rejection for both, verify no ref, authoritative/mergeable PR, or integration change was created, and retain the remote result evidence.

11. Generate the root-owned operator-prerequisite verification artifact binding the credential audit, revocations, remote policy, broker/non-broker identities, and real-remote negative test. Only after its digest is verified may the authority mode change from `evidence-only` to fully verified PR/push authority.

12. Prove direct `scripts/dispatch` invocation leaves authoritative evidence untouched and local push/PR spies at zero. Record explicitly that these spy tests are supplementary and do not substitute for step 10.

13. Produce a local commit through a direct run, poison its local evidence and review artifacts, and readmit only the commit through a fresh launcher attempt. Prove fresh identity, evidence, tests, and review. In fully verified mode, also prove fresh service-controlled ref, broker push, and broker PR handling; in evidence-only mode, prove no external PR/push result is represented as authoritative evidence.

14. Capture operator-owned metrics baselines before candidate metrics work.

15. Add extraction parity and non-metrics import-isolation regressions.

16. Extract metrics into `scripts/metrics_report.py` while preserving baseline behavior.

17. Add complete-history, terminal-window, zero-denominator, neutral-attribution, and five-spec semantic fixtures.

18. Implement corrected attribution and per-spec straight-through semantics.

19. Independently review, merge, install, and verify Dispatch 1.

20. Assign every required test a stable parent-owned test ID and exact expected assertion-ID set.

21. Classify substantive assertions as TRUST or non-TRUST.

22. Add a positive control and induced-failure mutation drill for every substantive TRUST assertion.

23. Implement the root-owned installed test source and explicit candidate target-root contract.

24. Implement the controller/candidate UID and process-namespace preflight.

25. Implement per-invocation nonces, separate trusted/supplemental channels, environment scrubbing, descriptor closure, and controller-only event production.

26. Add environment, descriptor, `/proc`, `ptrace`, process-VM, and `pidfd_getfd` forgery attacks.

27. Implement exact-set aggregation, single-launch accounting, and fail-before-review transitions.

28. Add post-test, pre-review, and pre-PR candidate-tree revalidation.

29. Persist the exact digest-approved acceptance-criteria vector at launch.

30. Implement strict JSON parsing, duplicate-key and nonstandard-constant rejection, schema v4, exact positional criteria binding, and legacy display-only handling.

31. Implement structured finding arrays and the defensive blocker predicate inside `evaluate_binary_review()`.

32. Add the machine attestation summary to the reviewer prompt and keep raw logs out of the prompt.

33. Add the real SPEC-015 state-machine replay with reviewer and PR spies.

34. Run every positive control, mutation drill, forged-event test, tree-mutation test, verdict-v4 case, and SPEC-015 replay on-box.

35. Independently review every trust-critical Dispatch 2 diff, including its tests and assertion inventory.

36. For any changed required trust test/helper/fixture, record explicit human authorization naming its old and new blob IDs.

37. Merge Dispatch 2, install the exact merged parent, provision its minimum gate environment, generate the new installation record, have the launcher verify the measured installed parent before dispatch, and rerun all on-box validation.

38. Revalidate the credential inventory, remote policy, bypass lists, and real-remote non-broker push/PR rejection against the final installed repository configuration. If any check fails or cannot run, record `evidence-only` mode and disable authoritative PR/push representation.

39. Mark Phase 2’s PLAN-003 prerequisite satisfied only after all three dispatches meet R8.7, with the authority mode and any `machine-account-authority-split` limitation stated explicitly.

## 7. Failure modes & blast radius

| Trigger | Consequence | Mitigation |
|---|---|---|
| Direct repository dispatcher is invoked | Local compute, files, worktrees, reviews, commits, and—before credential segregation—possibly external pushes or PRs may be created | Its evidence remains non-authoritative. Until the operator prerequisite passes, PR/push authority is explicitly not claimed; outside-broker PRs are never represented as evidence of gate passage. |
| A parallel operator Git/hosting credential remains usable | A direct dispatcher can bypass the broker, push a branch, and open a potentially mergeable PR while local spies remain zero | Keep `evidence-only` mode; revoke/remove the credential; repeat the complete inventory and real-remote negative authorization test before enabling the PR/push claim. |
| Remote service-ref or PR restrictions are absent, incomplete, or target the wrong repository/ref | Non-broker credentials can recreate an external authoritative path around the truthful gate | Remote branch/ruleset, push, PR, permission, and bypass enforcement is mandatory; real-remote push and PR rejection is the proof. |
| Operator retains remote admin or ruleset-bypass authority in an ordinary credential | The operator UID can undo or bypass the remote boundary | Revoke/remove that credential or role from the ordinary environment; require policy-changing authority outside the operator UID and record bypass-list state. |
| Real-remote negative test is replaced by spies, invalid credentials, or a network failure | Tests can pass without proving remote authorization | Require a verified authenticated non-broker identity and an authorization-layer remote rejection; transport/authentication failures are test failures. |
| Remote policy drifts after verification | A previously valid PR/push choke point may silently reopen | Bind policy identifiers/digests in the verification artifact; recheck before brokered authoritative operations and after installation; fall back to `evidence-only` on mismatch or unavailable verification. |
| An outside-broker PR passes `ci`, becomes mergeable, or is merged | Hosting state may look authoritative despite bypassing truthful gate evidence | Refuse to bind, import, attest, or represent it as authoritative evidence; preserve it as an incident artifact and route completion to `machine-account-authority-split`. |
| Direct-run artifacts are adopted | Untrusted claims could enter an authoritative attempt | Poisoned-artifact regression; fresh identity, evidence, tests, and review are mandatory, with fresh broker ref/PR operations only in fully verified mode. |
| Installed parent differs from its record | Wrong gate code or inventory could execute | The root-owned launcher measures the installed parent and performs an exact record comparison, refusing to launch dispatch on disagreement. |
...
The worst in-scope consequence is either incorrectly authorized evidence or, while the machine-account split is absent or has drifted, a control-plane PR created around the truthful gate and mistaken for authoritative. On detection, stop new launcher attempts, disable PR/push brokerage and authoritative PR representation, force `evidence-only` mode, preserve all local and remote evidence, inventory and revoke the bypass credential, restore remote enforcement, repeat the authenticated real-remote negative test, and reinstall the last independently reviewed known-good parent.
...
15. Push/PR spies remain at zero while a direct dispatcher using an existing operator credential can push a branch and open a mergeable PR on the real remote.
16. Root-only storage protects the broker credential while a Git helper, `gh` token, SSH key, cached token, deploy credential, or operator remote role supplies a parallel write/PR path.
17. A non-broker PR with `ci` success becomes mergeable because remote policy does not bind the authoritative path to the broker identity.

### 8.2 Launcher, installation, evidence choke-point, and real-remote authority validation

- Verify root ownership and worker write denial for installed parent content, installation records, the gate environment, evidence roots, credential storage, and operator-prerequisite verification artifacts.
- Mutate each recorded trusted component and require the root-owned launcher to refuse to launch the dispatcher.
- Present a parent whose measured tree does not match the installation record’s recorded commit and tree and require the root-owned launcher to refuse it at launch.
- Substitute the installation-record digest in a launch context and require refusal.
- Start without an operator-prerequisite artifact and prove the recorded mode is `evidence-only`, authoritative evidence remains service-only, and the kernel refuses to represent any outside-broker PR or push as gate-passage evidence.
- Inventory system/global/local/worktree/URL-scoped Git credential helpers and referenced stores; Git cache daemons/sockets; plaintext stores and `.netrc`; keyrings; `gh auth` hosts and tokens; token/askpass environment variables; shell/session/user-service credential sources; PAT/OAuth/app/deploy tokens; SSH agents, identities, keys, certificates, host aliases, `Include`/`Match` rules, and hardware-backed identities; deploy keys; repository roles; admin grants; and bypass lists.
- For every discovered non-broker write/PR/merge/admin/bypass path, record its disposition, revoke it remotely, remove it locally, and repeat discovery from a clean login with clean SSH agent, `gh`, Git, shell, and user-service contexts.
- Verify that the ordinary operator identity and every non-broker test identity are absent from remote write, PR-authority, merge, administrative, and ruleset-bypass grants.
- Verify remote branch protection/rulesets restrict creation and update of the exact service-controlled ref namespace to the broker/machine identity.
- Verify remote policy restricts the authoritative PR path to the broker/machine identity and prevents `ci` alone from making a non-broker PR authoritative or mergeable.
- Verify the remote repository ID, target branches, service-ref patterns, broker identity, allowed/bypass actor sets, ruleset IDs, and protection digests match the prerequisite artifact.
- Authenticate to the real remote as a dedicated non-broker identity and verify the remote-reported account identity before exercising authorization.
- Attempt to create a unique sacrificial service-controlled ref using the authenticated non-broker credential. Require an authorization rejection from the real remote. A local error, missing executable, intercepted call, invalid/expired token, DNS failure, TLS failure, or repository-not-found response does not satisfy the test.
- Query the real remote after the rejected push and prove the sacrificial service-controlled ref does not exist and the integration tip is unchanged.
- Using the same authenticated non-broker identity, attempt to create an authoritative PR or convert a non-broker PR into the authoritative mergeable path. Require the real remote policy to reject the operation.
- Query the real remote after the rejected PR operation and prove that no authoritative or mergeable PR, authority marker, protected service ref, or integration change was created. An ordinary PR object is acceptable only if remote policy makes it mechanically non-authoritative and unmergeable and the attempted authoritative transition itself was rejected.
- If either real-remote operation succeeds, cannot be executed safely, or fails below the authorization layer, require `evidence-only` mode, disable authoritative PR/push representation, and leave `machine-account-authority-split` unresolved.
- Generate the operator-prerequisite verification artifact from the successful audit, remote configuration, and remote negative test. Alter its repository ID, broker identity, policy digest, bypass list, test identity, or result and require refusal to enable full PR/push authority.
- Drift or remove a relevant remote rule after a valid artifact and require the broker’s pre-operation check to disable authoritative PR/push operations and revert to `evidence-only`.
- Invoke repository `scripts/dispatch` directly as the ordinary operator and as `codex-worker`; require authoritative evidence to remain unchanged and local push/PR spies to remain at zero.
- Confirm that the spies in the preceding test are supplementary dispatcher-integration checks only. They do not satisfy any credential-inventory, remote-policy, or real-remote rejection criterion.
- Confirm the direct run may create local non-authoritative effects and that no dispatcher self-check is claimed or required on that path.
- Before full PR/push verification, demonstrate that any externally created direct-run PR remains unbound and cannot be imported, labeled, attested, or represented as authoritative evidence, even if `ci` passes.
- Poison direct-run evidence, verdicts, attestations, test results, attempt identity, push metadata, and PR metadata; submit only its commit to the launcher; prove none of the poisoned artifacts or digests is adopted.
- Require a new attempt identity, fresh controller invocation records, and fresh review. In fully verified mode also require a service-controlled ref and broker-only push/PR operations; in `evidence-only` mode require external PR/push authority to remain disabled and unrepresented.
- Attempt to make `codex-worker` replace the installation record, inventory, required test, helper, fixture, schema, environment, or operator-prerequisite artifact and require denial.
...
- Record the replay’s authority mode. If R1.6–R1.9 are not currently verified, require `evidence-only` and prove the replay cannot represent any PR or push as authoritative evidence.
...
- **Revision 8:** Resolves the sole revision-7 stop-rule executability inconsistency by assigning installed-parent measurement and installation-record comparison to the root-owned launcher, without widening Dispatch 0’s repository paths into `scripts/dispatch.py`.

- **Revision 9:** Resolves the revision-8 security-material finding by separating the established authoritative-evidence boundary from the conditional PR/push boundary; making operator credential revocation, remote broker-identity enforcement, and authenticated real-remote negative authorization tests explicit prerequisites; demoting spies to supplementary checks; and recording `machine-account-authority-split` as the owner whenever the kernel remains in evidence-only mode.

- **Authorization:** Pending adversarial review under R24’s stop-rule and an authorized revision-9 digest. Zero security-material and executability-blocking findings authorizes the plan; advisory notes and already recorded residuals do not block it.
...
**Why deferred:** R24 replaces this machinery with operator-run installation, an installation record containing the installed parent commit and digests, and a root-owned launcher check that the measured parent matches that record before dispatch is launched.
...
**Why deferred:** R24 substitutes the authoritative-evidence choke point and independent human review of every trust-critical diff. PR/push exclusivity is not supplied by that substitution and is claimed only after the separate machine-account operator prerequisite passes.

**Risk accepted:** Candidate code cannot create authoritative evidence or reach broker credentials, but the new gate’s own tests are reviewed by humans rather than mechanically validated by a prior frozen verifier. Until the machine-account prerequisite passes, a direct operator run may retain external PR/push capability, though none of its results can be adopted as authoritative evidence.
...
### D12. Machine-account authority split

**Deferred item:** Completion, restoration, or later hardening of the exclusive machine-account boundary for service-controlled refs and authoritative PRs whenever the operator credential-segregation and remote-enforcement prerequisite is not installed or no longer verifies. This includes eliminating all parallel human/operator write and PR credentials, maintaining remote actor/ref/PR restrictions and bypass-list controls, and maintaining a repeatable authenticated real-remote authorization oracle.

**Why deferred:** PLAN-003 can close false-pass certification for authoritative evidence without falsely claiming that current repository PR/push authority is already segregated. The PR/push claim is enabled only when R1.6–R1.9 are verified; otherwise the kernel remains explicitly `evidence-only`.

**Risk accepted:** Before the split is verified, a direct operator run may use a parallel credential to push a branch or create a CI-passing or mergeable PR. PLAN-003 does not prevent that external effect. It prevents the effect, its PR, its status, or its merge from being adopted or represented as authoritative evidence of truthful gate passage.

**Fail-closed consequence:** Missing, failed, stale, or unverifiable credential inventory, remote enforcement, bypass-list state, or real-remote negative authorization evidence forces `evidence-only` mode. Brokered authoritative PR/push representation remains disabled, and any PR produced outside the broker path is rejected as evidence.

**Future-spec owner:** `machine-account-authority-split`.
...
2. **Manual installed-parent selection:** The launcher’s comparison proves that the installed parent admitted through the authoritative-evidence path matches the selected installation record, but it does not provide CAS selection, stale-parent exclusion, concurrent transition serialization, or durable activation history. Direct dispatcher invocation remains possible. Its artifacts cannot become authoritative evidence; its PR/push effects are not claimed to be choke-pointed until the separate operator prerequisite passes.
...
7. **Evidence sealing completeness:** The root-owned authoritative-evidence choke point prevents candidate-written authoritative evidence, but comprehensive sealing, hash chaining, and crash-durable no-replace behavior remain with the evidence-hashing plan. This claim does not establish PR/push exclusivity.
...
10. **Direct-run local and external effects:** Direct invocation may waste compute or create confusing local worktrees, processes, reviews, and commits. Before credential segregation it may also push or open a PR through a parallel operator credential. None of its evidence or external results can be represented as authoritative gate evidence, but operator and hosting-platform confusion remains possible.

11. **Privileged administrator threat:** Root-owned installation prevents candidate modification, not malicious or mistaken root replacement. Full closure and activation hardening are deferred. Separately, a hosting-platform administrator outside the ordinary operator environment can change remote policy; such a change invalidates the recorded PR/push authority until reverified.
...
14. **Conditional PR/push authority:** The PR/push choke point is not a property of root-owned broker storage alone. It exists only while non-broker credentials remain revoked, remote restrictions remain effective, bypass lists remain constrained, and the authenticated real-remote negative authorization test remains applicable. Failure or uncertainty in any element narrows the kernel to authoritative evidence authority and returns ownership to `machine-account-authority-split`.

## Disposition record (revision 9)

- **Changed — frontmatter:** `revision` advanced from 8 to 9.

- **Changed — §1:** The decision now distinguishes the unconditional authoritative-evidence choke point from conditional PR/push authority. It states the `evidence-only` limitation until credential segregation and remote enforcement are verified and adds the fail-closed rule that outside-broker PRs cannot be represented as authoritative evidence.

- **Changed — §3, R1 and R8.7:** R1 now requires a complete operator credential inventory and revocation/removal procedure, remote-side broker/machine-identity enforcement, an authenticated negative authorization test against the real remote, a bound verification artifact, explicit authority modes, and supplementary-only spy tests. R8.7 now carries the `machine-account-authority-split` limitation when those prerequisites are absent.

- **Changed — §4.1–§4.4 and §4.13:** Dispatch 0, completion semantics, allowed paths, the choke-point design, installed-parent claims, and rejected alternatives now distinguish evidence authority from conditional PR/push authority and specify both halves of the operator prerequisite.

- **Changed — §5:** Boundary and consumer rows now cover the operator credential environment, remote policy, real-remote test, authority-mode record, outside-broker PRs, and `machine-account-authority-split`.

- **Changed — §6:** Ordered implementation now installs `evidence-only` mode first; inventories, revokes, and removes parallel credentials; configures remote restrictions; executes real-remote push and PR rejection tests; records the prerequisite artifact; and revalidates it after final installation. Spy tests are explicitly supplementary.

- **Changed — §7:** Failure modes now cover surviving parallel credentials, missing or drifting remote policy, operator bypass authority, invalid negative-test substitutes, and outside-broker CI-passing or merged PRs. Incident handling now forces `evidence-only` mode and remote-policy remediation.

- **Changed — §8.1, §8.2, §8.8, and §8.9:** Validation now includes the pre-fix parallel-credential regression, exhaustive credential-source checks, remote configuration verification, authenticated real-remote push and authoritative-PR rejection, post-operation remote queries, drift handling, and authority-mode validation. Local push/PR spies are downgraded to supplementary checks and remain outside the proof of remote authorization.

- **Changed — §11:** Revision-9 provenance and authorization-digest wording were added.

- **Changed — Deferred D2 and new D12:** D2 no longer implies that the human-review substitution establishes PR/push exclusivity. D12 assigns the unresolved or regressed machine-account authority split to `machine-account-authority-split` and records its fail-closed `evidence-only` consequence.

- **Changed — Residual risks 2, 7, 10, and 11; added residual 14:** Residuals now scope the choke point to evidence until the operator prerequisite passes, acknowledge possible direct-run external effects, distinguish hosting administration from local root ownership, and record the conditional nature of PR/push authority.

All other revision-8 text is frozen verbatim.
