...
Dispatch-side capture is limited to one bounded nonblocking raw Unix datagram emission to the fixed local journald socket. It uses `AF_UNIX` with `SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC`, one preallocated bounded buffer, and exactly one `sendmsg`/`sendto` attempt with `MSG_DONTWAIT | MSG_NOSIGNAL`. It has no alternate address or transport fallback and performs no measurement file write, flush, `fsync`, acknowledgement, retry, helper invocation, or Claude wrapping. Each emission occurs only after the corresponding canonical transition is durable. A dropped datagram is a coverage gap and cannot affect dispatch.
...
21. **Semantic epoch:** `post-phase-1` requires trusted journald producer metadata, an unpredictable evaluation ID predeclared in the immutable run manifest where applicable, a trusted clean-checkout/runtime-code attestation whose file digests match the claimed harness commit, and a captured harness-commit binding to the pinned implementation and schema set. Git ancestry proof or a trusted finalized ancestry attestation is a contributing source. Otherwise the epoch is `unknown`. Trends and A/B reject unknown or mixed epochs.
...
- Boot ID.
- A trusted clean-checkout/runtime-code attestation whose file digests match the claimed harness commit.
...
For evaluation, the lifecycle record also binds an unpredictable run/invocation ID generated by the trusted runner and predeclared in the immutable run manifest before model invocation.

The collector accepts dispatcher-originated capture only when the trusted journald fields `_UID`, `_SYSTEMD_UNIT`, `_SYSTEMD_CGROUP`, `_EXE`, `_PID`, boot ID, and available process-start identity match the pinned operator dispatcher profile and the runtime attestation. Client-supplied copies of those fields are ignored as authentication evidence.

The source set includes the raw materialized capture record with trusted journald metadata, runtime-code attestation, immutable run manifest where applicable, relevant Git commit objects or finalized ancestry attestation, extractor/schema contracts, and semantic-boundary evidence.

Absent, untrusted, or mismatched producer metadata, manifest binding, runtime attestation, or ancestry proof yields `unknown`.
...
When Git ancestry affects `semantic_epoch`, the source set additionally binds the relevant commit objects and deterministic ancestry-command output, or a trusted finalized ancestry attestation that binds those objects and the ancestry rule.
...
- Emits only after the corresponding canonical transition and evidence write are durable.
...
The production Claude call is unchanged. After the existing call returns and its corresponding canonical result transition is durable, dispatch may emit a bounded Claude-completion datagram containing scalar metadata and source digests. It never routes Claude through a wrapper.

Syscall-trace tests assert, for success and every injected failure, at most one attempted datagram send, no retry, no alternate socket/address, and no measurement or fallback file operation. Call-site tests inject cancellation and exceptions between transitions and verify that no record is emitted before canonical durability.
...
Materialization uses a temporary file and atomic rename outside dispatch. Each record retains journal cursor, boot ID, realtime/monotonic timestamp, trusted producer metadata, record digest, raw class, run-manifest identity where applicable, and coverage.
...
- The trusted producer and runtime-code attestations required for epoch classification.
...
- `matrix_result` stores scheduled pairs, paired count, one-sided missing counts, scheduled/contributing/zero-pair task identities, arm-specific missingness, holdout IDs, causal hashes, and the statistical-contract digest.
...
- The trusted clean-checkout/runtime-code attestation matches the claimed harness commit.
...
Each scheduled evaluation invocation receives an unpredictable ID generated by the trusted runner and recorded in the immutable run manifest before invocation. Capture and finalized result records must bind that exact ID.
...
|---|---|
| `scripts/dispatch.py` | Only `P2-02` touches it; change is limited to one fixed-address raw nonblocking journald datagram after canonical durability. |
...
The high-assurance closure for `P2-02` is limited to the exact emission helper, its `scripts/dispatch.py` call sites, raw-schema compatibility, durability ordering, syscall-trace proof, and failure-equivalence tests.
...
- Git ancestry evidence or its trusted finalized attestation is present in the source set whenever epoch depends on ancestry.
...
### 2. Generic journald materialization and post-hoc Claude join — `P2-03`, ordinary
...
- Emit lifecycle, concurrency, and post-call Claude metadata at existing transitions only after the corresponding canonical transition is durable.
...
- Requirements 4, 5, and 17–21 pass.
...
- Call-site fault injection proves no emission occurs before the corresponding canonical evidence is durable.
- No dispatch invocation writes journal, SQLite, report, or capture files.
- Lifecycle, concurrency, and Claude records pass end to end through `P2-03` materialization, journal event, and projection before authorization.
...
- Produce the trusted clean-checkout/runtime-code attestation used by epoch validation.
...
- Runtime file digests match the claimed clean harness commit or epoch remains `unknown`.
...
- Baseline records configuration, harness binding, runtime-code attestation, boundary, dataset, scorer, holdout registry, authorization, generation, and checkpoint digests.
...
| Ancestry proof changes or disappears | Epoch misclassification | Bind commit objects or trusted finalized ancestry attestation as sources. |
...
| Dirty/substituted runtime self-reports a valid commit | False epoch | Require clean-checkout/runtime-code attestation matching commit bytes. |
...
| Emission precedes durable transition | Capture contradicts canonical evidence | Emit only after canonical durability; call-site fault tests. |
| Lifecycle/concurrency datagrams are not materialized | Missing promised streams | Generic `P2-03` materializer and per-class end-to-end tests. |
...
| 17–19 | Exactly four raw schemas, generic three-class journald materialization, syscall tracing, durability ordering, and full fault injection. |
| 20–21 | UID/filesystem canaries, trusted producer/runtime attestation, manifest-ID binding, ancestry source binding, and epoch rejection. |
...
- Syscall tracing proves no alternate socket/address, library/helper transport, retry, queue, fallback write, or measurement file operation.
- Call-site fault injection proves emission occurs only after the corresponding canonical transition and evidence write are durable.
...
- Clean-checkout/runtime-code attestations match the claimed harness commits.
- Git ancestry proof or its trusted finalized attestation is included in the source set.
...
- **Finding 1 — SUSTAINED.** Removed `sd_journal_sendv`. Dispatch capture is now one raw `AF_UNIX` datagram to fixed address `/run/systemd/journal/socket`, using `SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC`, one preallocated buffer, and one `sendmsg`/`sendto` attempt with `MSG_DONTWAIT | MSG_NOSIGNAL`. There is no alternate address, transport, helper, retry, queue, acknowledgement, or fallback. Emission occurs only after the corresponding canonical transition is durable. Syscall tracing proves the send count, flags, address, and absence of file/fallback operations under every injected failure. Sections changed: §1 dispatch capture, requirements 4–5 and 18–19, §§4.7, 5, 6 (`P2-02`), 7, 8.2, 8.4, and 9.

- **Finding 2 — SUSTAINED.** Added `extractor_implementation_sha256` for the actual extractor/canonicalizer semantic implementation closure and `semantic_payload_sha256` for deterministic semantic event content. Both bind event identity. Duplicate event IDs require byte-identical deterministic content; a mismatch seals the generation as `identity_collision`. Git ancestry proof, including relevant commit objects or a trusted finalized ancestry attestation, is a contributing source whenever epoch depends on it. Sections changed: §1, requirements 9–11, §§4.2–4.6, 4.9–4.10, 6 (`P2-01`, `P2-05`), 7, 8.2–8.3, and 9.

- **Finding 3 — SUSTAINED.** Dispatcher-originated records now require trusted journald producer UID, unit/cgroup, executable, PID/start identity, and boot ID. A D5 worker-forgery canary must fail closed. Evaluation records bind unpredictable invocation IDs predeclared in immutable run manifests. `post-phase-1` additionally requires a trusted clean-checkout/runtime-code attestation whose file digests match the claimed harness commit; otherwise the epoch is `unknown`. All provenance, manifest, runtime-attestation, and ancestry records join the source set. Sections changed: requirement 21, §§4.6–4.7, 4.11, 4.15, 5, 6 (`P2-03`, `P2-02`, `P2-06`, `P2-08`, `P2-09`), 7, 8.2, 8.4–8.6.

- **Finding 4 — SUSTAINED.** Collector success now requires a durable immutable checkpoint candidate for the exact append head. Append/checkpoint file and directory `fsync` ordering and crash recovery are specified. A complete valid unacknowledged tail may receive its missing checkpoint without rewriting the generation; invalid tails seal the generation. Truncation claims are narrowed to retained checkpointed heads, and coordinated rollback of both journal and unanchored checkpoint is recorded as a limitation. The circular containing-commit field was removed; Git or a later external anchor records the containing commit. Sections changed: §1, requirements 6–8, §§4.2, 5, 6 (`P2-01`, `P2-09`), 7, residual risks, 8.2–8.3, 8.6, and 9.

- **Finding 5 — SUSTAINED.** `P2-03` is now the generic journald materializer for lifecycle, concurrency, and Claude-envelope classes. The archived-artifact Claude join remains an additional post-hoc step over materialized Claude records. Per-class fixtures and end-to-end tests from raw datagram through immutable source, journal event, SQLite, and report are mandatory before `P2-02` authorization. Sections changed: §1, requirements 17–19, §§4.7, 5, 6 (`P2-03`, `P2-02`), 7, 8.2, and 8.4.

- **Finding 6 — SUSTAINED.** Replaced independent repeat hash bits with the exact counterbalancing formula:
  `initial_bit = low_bit(sha256(run_id NUL task_id))`;
  `order_bit = initial_bit XOR ((repeat_index - 1) mod 2)`.
  This guarantees 1/1 ordering for two repeats and 2/1 or 1/2 ordering for three repeats. Sections changed: requirement 40, §4.15 Ordering, 6 (`P2-08`), 7, 8.2, and 8.5.

- **Finding 7 — SUSTAINED.** Every aggregate metric now predeclares its estimand, task/repeat weighting, missing policy, zero-complete-pair policy, ITT value where applicable, and differential-missingness threshold. Safety and terminal metrics retain ITT denominators. Nullable efficiency metrics use a labelled equal-task-weight eligible-task estimand; zero-pair tasks are explicitly listed, an empty contributing set is `non_estimable`, and missing efficiency values never become zero. Every interval reports scheduled/contributing/zero-pair task identities and arm-specific missingness. Directional efficiency conclusions are prohibited above the hashed default absolute missing-rate threshold of `0.0500` or a stricter predeclared value. Sections changed: requirements 37–38, §§4.9–4.10, 4.15 Missing and failed runs/Aggregate statistics, 6 (`P2-08`, `P2-09`), 7, 8.2, 8.5–8.6.

- **Execution-order contradiction — SUSTAINED.** Phase 1 owns the immutable committed generated extractor contract and produces it as part of Phase 1 Dispatch 5 completion evidence. The Phase 1 precondition and evidence pin require it before any Phase 2 implementation authorization. `P2-01` only verifies and may vendor a byte-identical copy; it cannot generate or modify the contract. Sections changed: §1 Phase 1 precondition, §§2.3, 4.3, 5, 6 (`P2-01`), 7, and 8.2.
