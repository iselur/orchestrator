...
29. **Bundle closure and label isolation:** a fixture bundle contains exactly one intended ref and its complete intended object closure, with no hidden check, oracle, label, manifest, or extra object. Runtime worktree, ref, branch, and systemd-unit names are opaque controller-generated identifiers and contain no descriptive task ID, category, probe, expected outcome, or label. Closure validation checks ref names and targets, annotated-tag metadata, commit metadata, tree entry names and modes, and blob contents for hidden material or label leakage.
...
37. **Immutable statistical contract and derived direction:** before invocation, every metric defines a schema-validated immutable statistical contract containing its exact estimand, task/repeat weighting, missing-value and zero-complete-pair policies, exact direction statistic and computation, orientation identifying which sign favors which arm, canonical quantization and rounding mode, tie region and boundary rule, explicit zero and negative-zero handling, any interval-based direction criterion, and every estimability, missingness, epoch, and publication gate. Per-task n=2–3 reports raw paired differences and descriptive ranges, never confidence intervals. Aggregates use the predeclared estimand and hierarchical resampling contract. The runner mechanically derives exactly one structured `derived_outcome` value—`candidate_favorable`, `baseline_favorable`, `no_direction`, or `non_estimable`—from the computed aggregate and contract. The field is never accepted from a human assertion or configuration label.

38. **Missing-run and direction-gate honesty:** output records scheduled pairs, `paired_n`, both one-sided-missing counts, missing reasons, intent-to-treat outcomes, scheduled and contributing task IDs/counts, zero-pair tasks, and arm-specific missingness for every aggregate metric. An uncomputable estimand or failed estimability gate yields `non_estimable`. An estimable aggregate whose rounded direction statistic is zero or within the declared tie region, whose interval criterion is not satisfied, or whose missingness, epoch, or publication direction gate fails yields `no_direction`. Differential efficiency missingness above the predeclared threshold is such a direction gate and cannot yield either favorable outcome.

39. **Percentage deltas and contradiction rejection:** `delta_percent` is null for bounded scores, counts, rates, unknown/nonpositive baselines, and baselines below metric-specific meaningful thresholds. `eval/statistical-contract.schema.json`, `eval/matrix.schema.json`, and `eval/delta.schema.json` reject undeclared direction rules, mutable or prose-only direction assertions, and outcome values outside the four-value enum. Semantic validation recomputes `derived_outcome` from the aggregate and immutable contract and rejects any stored outcome, label, prose, report text, or publication direction that contradicts it. Any rendered direction text is generated from a fixed template keyed only by the recomputed field.

40. **Counterbalance and edge cases:** arm order uses `initial_bit = low_bit(sha256(run_id NUL task_id))` and `order_bit = initial_bit XOR ((repeat_index - 1) mod 2)`, guaranteeing order counts within each task differ by at most one. Acceptance fixtures cover positive and negative orientations, exact ties, rounded ties, positive and negative zero, interval overlap with the declared tie region, empty contributing sets, missingness-gate failure, and epoch/publication-gate failure; each must resolve mechanically to the contract-declared one of the four outcomes.
...
eval/
  golden/v1/
    ...
    tasks/<task-id>/
      ...
      hidden/run
  matrix.schema.json
  statistical-contract.schema.json
  delta.schema.json
  controller-attestation.schema.json
...
eval/results/<run-id>/
  manifest.json
  controller-attestations/<invocation-id>.json
  results.jsonl
  delta.json
  delta.csv
...
`eval/results/<run-id>/manifest.json`, `controller-attestations/<invocation-id>.json`, `results.jsonl`, `delta.json`, and reviewer-experiment manifests are finalized collector sources. Raw prompts, model prose, transient logs, and temporary worktrees remain ignored.
...
- A trusted clean-checkout/runtime-code attestation produced by the `P2-06` evaluation controller, validated under `eval/controller-attestation.schema.json` and finalized at `eval/results/<run-id>/controller-attestations/<invocation-id>.json`, whose file digests match the claimed harness commit and bind the invocation ID and runtime identity.
...
- `matrix_result` stores scheduled pairs, paired count, one-sided missing counts, scheduled/contributing/zero-pair task identities, arm-specific missingness, holdout IDs, causal hashes, the immutable statistical-contract digest, the computed direction statistic and interval inputs, the mechanically recomputed four-value `derived_outcome`, and post-run epoch-publication validity.
...
Projection reads journal lines in chain order. Any checkpoint, chain, schema, identity-collision, statistical-outcome contradiction, or constraint failure deletes the temporary database and leaves the prior cache untouched.
...
- Each metric’s mechanically recomputed `derived_outcome`; any direction label or prose is emitted only from the fixed template keyed by that field.
...
- Exact intended Git ref, its peeled target, and complete reachable object closure, including ref bytes, object type, object ID, content digest, commit/tag metadata, and tree entry names and modes.
...
6. Inspect ref names and targets, annotated-tag names/messages and tagger metadata, commit author/committer/message and parent/tree headers, every tree entry name/mode/object identity, and every blob against the controller-side forbidden-material contract. Reject hidden checks, oracles, labels, manifests, undeclared paths, descriptive task IDs/categories/probes/outcomes, or other label-bearing metadata anywhere in that closure.
...
- Opaque controller-generated worktree-directory, Git-ref, branch, capture, and systemd-unit names that contain no descriptive task ID, category, probe, expected outcome, or label.
...
- Runtime worktree, ref, branch, capture, and unit names are opaque and cannot disclose descriptive task identifiers or labels.
...
- Fixture object closure is exact and its ref, tag, commit, tree, and blob metadata pass the forbidden-material scan.
...
The controller validates each attestation against the closed `eval/controller-attestation.schema.json` and finalizes it by atomic rename to:

```text
eval/results/<run-id>/controller-attestations/<invocation-id>.json
```

The artifact binds schema digest, run and invocation IDs, harness commit and clean-checkout state, runtime-file closure, implementation and schema-set digests, semantic boundary, ancestry evidence, opaque runtime worktree/ref/unit identities, UID/unit/cgroup/executable/PID/start/boot identity, creation/finalization state, and controller implementation digest. Unknown fields, a reused path or invocation ID, a non-final state, or any manifest/runtime mismatch fails closed.
...
After all scheduled runs finalize, post-run validation joins each invocation’s capture, runtime identity, finalized `eval/results/<run-id>/controller-attestations/<invocation-id>.json`, ancestry evidence, and finalized result. Comparison publication requires every scheduled invocation to resolve to the same `post-phase-1` epoch and boundary. A missing or mismatched post-run record makes the comparison non-publishable without deleting the finalized outcomes.
...
For every aggregate metric, the immutable statistical contract validated by `eval/statistical-contract.schema.json` predeclares:

- Metric family.
- Estimand.
- Exact aggregate point-estimate computation.
- Task weighting.
- Repeat weighting.
...
- Exact `direction_statistic`, including the aggregate field or deterministic computation from which it is obtained.
- `orientation`, expressed as `positive_favors: candidate|baseline`; the opposite nonzero sign favors the other arm.
- Canonical comparison scale, decimal quantization, and rounding mode. Version 1 permits only integer comparison where the metric is integral or four-decimal fixed-point comparison using round-half-to-even.
- A closed tie region with exact lower and upper decimal bounds and inclusive/exclusive boundary flags.
- The zero rule. Canonical positive zero and negative zero both compare as zero after quantization and must lie in the tie region.
- `interval_criterion`, exactly one of `none` or `entire_hierarchical_interval_outside_tie_region_in_point_estimate_direction`. When the latter is selected, the contract names the interval quantiles and strict/inclusive boundary comparison; both interval endpoints must lie beyond the same declared tie boundary in the point-estimate direction.
- Every estimability gate and its failure result.
- Every missingness, epoch, and publication direction gate and its failure result.
- The fixed direction-rendering template version.

No field equivalent to a free-standing permission to assert direction exists. In particular, `directional_conclusions_permitted` is prohibited by the closed schemas.
...
- Statistical-contract digest.
- Unrounded and canonically rounded direction statistic.
- Applied tie, interval, estimability, missingness, epoch, and publication-gate results.
- Exactly one mechanically derived `derived_outcome`.

The default differential-missingness threshold for nullable efficiency metrics is an absolute arm missing-rate difference of `0.0500`. It is schema-declared and hashed into the statistical contract. A metric may use a stricter predeclared threshold but not a looser one. If the observed absolute difference exceeds the threshold, that direction gate fails and the mechanically derived outcome is `no_direction`; the report displays only coverage and descriptive values.
...
2. If the contributing set is empty, do not resample and derive `non_estimable`.
...
5. Compute the requested aggregate and exact predeclared direction statistic.
...
10. Quantize the direction statistic and required interval bounds under the contract’s declared comparison scale and rounding rule, canonicalizing negative zero to zero.
11. Evaluate every estimability gate. A failed gate whose contract result is `non_estimable` derives `non_estimable`.
12. Evaluate every missingness, epoch, and publication direction gate. Any failed direction gate derives `no_direction`.
13. If the rounded statistic is zero or falls within the declared tie region, derive `no_direction`.
14. Otherwise determine the provisional favorable arm solely from the statistic sign and `positive_favors` orientation.
15. If `interval_criterion` is `entire_hierarchical_interval_outside_tie_region_in_point_estimate_direction`, require both declared interval endpoints, after canonical rounding, to lie beyond the applicable tie boundary in that same provisional direction. If they do not, derive `no_direction`.
16. Otherwise derive `candidate_favorable` or `baseline_favorable` from the provisional arm.
17. Store exactly that derived value and recompute it during schema-adjacent semantic validation, collection, projection, and report generation.

`derived_outcome` is never read from configuration as an assertion and is never selected by an operator or model. An immutable finalized artifact may store it only as a redundant derived field checked against its aggregate inputs and contract. A mismatch invalidates that artifact.

The schemas allow no arbitrary direction label or prose. If a finalized or rendered artifact contains direction text, it must be generated from a versioned fixed template keyed only by `derived_outcome`; semantic validation rejects text or labels inconsistent with the recomputed field. `no_direction` and `non_estimable` cannot be rendered as either arm favorable.
...
- Requirements 4, 5, and 17–20 pass.
- For requirement 21, `P2-02` proves only its available negative and fixture coverage: production capture remains `unknown`, forged or worker-originated capture cannot promote an epoch, and the emitted raw records preserve the manifest-ID and producer fields required by the already accepted offline evaluation fixtures. Live evaluation promotion is not accepted by `P2-02`; it waits for the `P2-06` controller-attestation mechanism and `P2-08` post-run validation.
...
eval/controller-attestation.schema.json
eval/score
eval/score.py
...
- Inspect ref, annotated-tag, commit, tree, and blob metadata for hidden material or descriptive label leakage.
- Implement the sealed D5 evaluation profile for worker and reviewer using opaque controller-generated runtime worktree, ref, branch, capture, and systemd-unit names.
- Run hidden oracles only from the trusted controller.
- Add fail-closed production-side-effect and capture-forgery canaries.
- Implement the closed `eval/controller-attestation.schema.json`.
- Produce the trusted evaluation-controller clean-checkout/runtime-code attestation used by evaluation epoch validation and atomically finalize one artifact per invocation at `eval/results/<run-id>/controller-attestations/<invocation-id>.json`.
...
- Runtime worktree, ref, branch, capture, and systemd-unit names are opaque and contain no descriptive task ID, category, probe, expected outcome, or label.
- A D5 worker cannot produce a record accepted as dispatcher-originated.
- Extra refs/objects and hidden or label-bearing content in ref names/targets, annotated-tag metadata, commit metadata, tree metadata, or blobs are rejected.
- A failed canary causes zero model invocations.
- Reused worktrees and changed fixture bytes are rejected.
- Every controller attestation validates under `eval/controller-attestation.schema.json`, is finalized only at `eval/results/<run-id>/controller-attestations/<invocation-id>.json`, and binds exactly one predeclared run/invocation ID.
...
eval/statistical-contract.schema.json
...
- Validate each immutable per-metric contract under `eval/statistical-contract.schema.json`; prohibit any free-standing permission flag or human/config direction assertion.
- Derive `derived_outcome` mechanically from each computed aggregate, orientation, rounding/tie/zero rule, interval criterion, and estimability/missingness/epoch/publication gates.
- Recompute and validate the derived field under `eval/delta.schema.json`; reject any contradictory outcome, label, prose, or publication direction.
...
- Every metric declares its estimand, task/repeat weighting, missing policy, zero-pair policy, exact direction statistic and sign orientation, comparison scale and rounding, tie and zero rules, any interval criterion, and every missingness/epoch/publication gate before invocation.
- Aggregate resampling is deterministic at 10,000 draws and yields `non_estimable` for an empty contributing set.
...
- A fixture with an estimable positive point estimate whose declared interval criterion crosses or touches the tie region yields `no_direction`.
- Exact ties, rounded ties, positive zero, negative zero, and non-estimable cases resolve exactly under the immutable declared rules.
- Differential missingness above `0.0500` or a stricter predeclared threshold yields `no_direction`, never either favorable outcome.
- A finalized fixture attempting to publish `candidate_favorable` or candidate-favorable prose when recomputation yields `baseline_favorable`, `no_direction`, or `non_estimable` is rejected; the symmetric baseline contradiction is also rejected.
- Stored `derived_outcome` is exactly one of `candidate_favorable`, `baseline_favorable`, `no_direction`, or `non_estimable` and equals independent recomputation.
...
- Every aggregate metric identifies its immutable statistical-contract digest, estimand, scheduled/contributing/zero-pair tasks, arm-specific missingness, exact direction statistic and orientation, rounded statistic, interval inputs, applied gates, and mechanically derived four-value `derived_outcome`.
- Baseline publication rejects any direction label or prose that does not match independent recomputation of `derived_outcome`.
- Baseline fixtures prove that an aggregate unsupported by its declared interval/tie rule publishes `no_direction`, and that ties, zeros, and non-estimable aggregates retain their declared outcomes without favorable prose.
...
| Descriptive runtime or Git metadata reveals a task label | Benchmark leak before model invocation | Opaque runtime worktree/ref/branch/unit names plus ref/tag/commit/tree/blob metadata scan. |
...
| Task has zero complete pairs for a metric | Undefined resampling or favorable exclusion | Predeclared metric estimand; list zero-pair tasks; derive `non_estimable` where required. |
| Differential efficiency missingness | Misleading cost direction | Failed direction gate mechanically derives `no_direction` above the predeclared threshold. |
| Aggregate point estimate does not satisfy its interval/tie rule | Unsupported favorable claim | Mechanically derive `no_direction` from the immutable contract. |
| Stored label or prose contradicts the computed aggregate | False directional publication | Recompute `derived_outcome`; closed schemas and semantic validation reject the artifact. |
...
| 20–21 | `P2-01` epoch-contract/offline fixtures; UID/filesystem canaries; `P2-02` negative/fixture coverage only; production forced to `unknown`; `P2-06` finalized `eval/results/<run-id>/controller-attestations/<invocation-id>.json` validated under `eval/controller-attestation.schema.json`; manifest-ID and ancestry source binding; `P2-08` preflight boundary/config validation and live post-run promotion/publication validation. |
...
| 27–29 | Merkle/object-closure, opaque runtime naming, ref/tag/commit/tree/blob metadata scanning, and sealed-profile canaries. |
...
| 37–40 | Raw differences; immutable per-metric direction contracts; exact statistic orientation, rounding, tie/zero/interval and gate rules; hierarchical resampling; missing accounting; mechanical four-value outcome derivation; contradiction rejection; and exact XOR AB/BA. |
...
- Requirement 21 validation here is limited to negative and fixture coverage; live evaluation promotion remains pending `P2-06` and `P2-08`.
...
- Runtime worktree, ref, branch, capture, and systemd-unit names are opaque controller-generated identifiers and disclose no descriptive task ID, category, probe, expected outcome, or label.
- Bundle pack equals the exact intended object closure.
- Ref names/targets, annotated-tag metadata, commit metadata, tree entry metadata, and blobs are scanned; hidden/oracle/label material and descriptive task identifiers are absent.
...
- The trusted `P2-06` controller validates against `eval/controller-attestation.schema.json` and atomically finalizes one evaluation-only clean-checkout/runtime-code attestation at `eval/results/<run-id>/controller-attestations/<invocation-id>.json`, binding the claimed harness commit, invocation ID, runtime identity, runtime-file closure, schema set, semantic boundary, ancestry evidence, and opaque runtime names.
...
- Every metric validates under `eval/statistical-contract.schema.json` and predeclares its estimand, weighting, missingness, zero-pair handling, exact direction statistic and sign orientation, rounding, tie/zero rules, interval criterion, and every missingness/epoch/publication gate.
- A fixture whose point estimate does not satisfy its declared interval criterion yields `no_direction`.
- Exact ties, rounded ties, positive zero, negative zero, no-contributor cases, and other non-estimable cases yield their mechanically declared outcomes.
- Efficiency missingness above the declared threshold yields `no_direction`.
- Independent recomputation produces exactly one of `candidate_favorable`, `baseline_favorable`, `no_direction`, or `non_estimable`.
- `eval/delta.schema.json` and semantic validation reject a stored outcome, label, prose string, or publication direction contradicting the recomputed field.
...
- Every aggregate’s statistical-contract digest, direction statistic, orientation, rounding/tie/zero rule, interval inputs, gate results, and recomputed four-value `derived_outcome` reconcile across finalized source, journal, SQLite, JSON, and HTML.
- A fixture whose aggregate does not support direction publishes `no_direction`.
- Ties, zeros, and non-estimable aggregates publish no favorable direction.
- Any attempted direction label or prose contradicting the recomputed field is rejected before publication.
...
- **Blocking finding — SUSTAINED.** Replaced the free-standing directional-permission model with an immutable, schema-validated per-metric contract defining the exact direction statistic and computation, sign orientation, canonical rounding, tie/zero handling, optional interval criterion, and all estimability, missingness, epoch, and publication gates. The runner now mechanically derives exactly one of `candidate_favorable | baseline_favorable | no_direction | non_estimable`; schemas, semantic validation, projection, and reports reject any contradictory stored outcome, label, prose, or publication direction. Added required unsupported-direction, contradiction, tie, zero, interval, and non-estimable fixtures. Changed requirements 37–40, §§4.1, 4.9–4.10, 4.15, §6 (`P2-08`, `P2-09`), §7, and §§8.2, 8.5–8.6.

- **Advisory 1 — APPLIED.** Required opaque controller-generated runtime worktree, ref, branch, capture, and systemd-unit names, and extended fixture leakage checks across ref targets, annotated-tag metadata, commit metadata, tree entries, and blobs. Changed requirement 29, §4.11, §6 (`P2-06`), §7, and §§8.2 and 8.5.

- **Advisory 2 — APPLIED.** Narrowed `P2-02`’s requirement-21 claim to negative and fixture coverage; live evaluation epoch promotion explicitly waits for the `P2-06` controller mechanism and `P2-08` post-run validation. Changed §6 (`P2-02`) and §§8.2 and 8.4.

- **Advisory 3 — APPLIED.** Named the closed controller-attestation schema as `eval/controller-attestation.schema.json` and the finalized per-invocation artifact path as `eval/results/<run-id>/controller-attestations/<invocation-id>.json`, with explicit source binding and acceptance. Changed requirement 21, §§4.1, 4.6, 4.11, 4.15, §6 (`P2-06`, `P2-08`), and §§8.2 and 8.5.
