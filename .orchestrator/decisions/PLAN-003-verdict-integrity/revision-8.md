...
11. **R1.11:** The new blob remains supplemental during the candidate attempt that introduces it. It becomes authoritative only after merge, independent human review, explicit old/new-blob authorization, operator installation, and launcher-side installation-record verification.
...
7. **R8.7:** Phase 2 remains blocked until all three PLAN-003 dispatches are merged where applicable, independently reviewed, operator-installed, launcher-side installation-record verified, and validated through the required on-box checks.
...
This verification is performed by the root-owned launcher, which is outside candidate/operator write authority. It prevents a mismatched or substituted parent from being launched through the authoritative path. It does not make the dispatcher self-verifying: the dispatcher does not verify its own installation record. A direct non-launcher dispatcher invocation remains possible, but it is non-authoritative under the choke point and cannot produce adoptable authoritative evidence or reach PR/push authority.
...
|---|---|
| Root-owned launcher/service | Becomes the exclusive authority for attempt identity, authoritative evidence allocation, installed-parent selection and verification, and PR/push operations. |
...
| Direct repository dispatcher | May produce only local non-authoritative effects without a launcher-issued context; it does not verify its own installation record. |
...
5. Generate the installation record, configure the root-owned launcher to measure and compare the installed parent before launching dispatch, and verify worker write denial across the installed parent, record, environment, authoritative evidence root, and credential store.
...
31. Merge Dispatch 2, install the exact merged parent, provision its minimum gate environment, generate the new installation record, have the launcher verify the measured installed parent before dispatch, and rerun all on-box validation.
...
- Confirm the direct run may create only local non-authoritative effects and that no dispatcher self-check is claimed or required on that path.
...
- **Authorization:** Pending adversarial review under R24’s stop-rule and an authorized revision-8 digest. Zero security-material and executability-blocking findings authorizes the plan; advisory notes and already recorded residuals do not block it.
...
2. **Manual installed-parent selection:** The launcher’s comparison proves that the installed parent admitted through the authoritative path matches the selected installation record, but it does not provide CAS selection, stale-parent exclusion, concurrent transition serialization, or durable activation history. Direct dispatcher invocation remains possible and non-authoritative.
...
## Disposition record (revision 8)

- **Changed — frontmatter:** `revision` advanced from 7 to 8.

- **Changed — §1, fourth paragraph:** Installed-parent measurement and installation-record comparison moved from the dispatcher to the root-owned launcher. The text now states explicitly that the launcher refuses mismatched parents on the authoritative path, the dispatcher does not verify its own installation record, and direct non-launcher invocation remains possible but non-authoritative.

- **Changed — §3, R1.3, R1.4, R1.11, and R8.7:** Acceptance criteria now assign installed-parent verification and mismatch refusal to the launcher and identify recorded parent measurements as launcher measurements.

- **Changed — §4.1:** The completion definition now identifies installation-record verification as launcher-performed.

- **Changed — §4.4:** The installed-parent check is now wholly launcher-side and its guarantees and limits are stated explicitly.

- **Changed — §5:** The root-owned launcher/service and direct repository dispatcher rows now identify launcher-side verification and the absence of dispatcher self-verification.

- **Changed — §6, steps 5 and 31:** Implementation now configures and exercises launcher-side installed-parent measurement and comparison.

- **Changed — §7, “Installed parent differs from its record” row:** The mitigation now targets launcher refusal to launch dispatch.

- **Changed — §8.2:** Validation now targets the launcher check, includes refusal of a parent whose measured tree does not match the installation record, and confirms that the direct non-authoritative path carries no dispatcher-self-check claim.

- **Changed — §11:** Revision-8 provenance and authorization-digest wording were added.

- **Changed — D1 “Why deferred”:** The replacement for the activation state machine is now identified as a root-owned launcher check, not a dispatcher check.

- **Changed — Residual risk 2:** The residual now states precisely what launcher-side comparison proves and preserves the non-authoritative status of direct dispatcher invocation.

All other revision-7 text is frozen verbatim.
